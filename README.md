# HueCIT Chatbot RAG

Hệ thống chatbot hỗ trợ kỹ thuật vận hành phần mềm sử dụng RAG (Retrieval-Augmented Generation) và phân tích ảnh lỗi.

## Tổng quan

Dự án xây dựng chatbot với hai năng lực chính:
1. **Hỏi đáp văn bản** — Trả lời câu hỏi hướng dẫn sử dụng dựa trên tài liệu nội bộ (RAG pipeline).
2. **Phân tích ảnh lỗi** — Nhận ảnh screenshot lỗi, nhận dạng ngữ cảnh chức năng/lỗi, và đưa hướng dẫn khắc phục.

**Thời gian**: 8 tuần (2 tháng) | **Team**: 2 người

## Quick Start

### Yêu cầu
- Python 3.9+
- 16GB+ RAM
- 50GB+ disk space

### Cài đặt
```bash
# Clone repo
git clone https://github.com/TienDat11/huecit-chatbot-rag.git
cd huecit-chatbot-rag

# Tạo virtual environment
python -m venv venv
# Windows:
venv\Scripts\activate
# Linux/Mac:
source venv/bin/activate

# Cài dependencies
pip install -r requirements.txt
pip install -r requirements-dev.txt

# Cài pre-commit hooks
pre-commit install
```

### Chạy tests
```bash
# Chạy tất cả tests
make test

# Chạy tests cho tuần cụ thể
pytest tests/week1/ -v
```

### Quality commands
```bash
make lint          # Chạy linting
make format        # Format code
make test          # Chạy tests
make check         # Chạy tất cả checks
```

## Cấu trúc dự án

Xem chi tiết tại [docs/REPO_STRUCTURE.md](docs/REPO_STRUCTURE.md).

```
huecit-chatbot-rag/
├── src/           # Source code (week1-week8)
├── tests/         # Test suite
├── data/          # Dữ liệu đầu vào
├── output/        # Kết quả pipeline
├── notebooks/     # Jupyter notebooks
├── docs/          # Tài liệu
└── scripts/       # Helper scripts
```

## Epics

| Epic | Mô tả | Tuần | Status |
|------|--------|------|--------|
| A | Setup & Governance | 1 | COMPLETE |
| B | Data Readiness | 1-2 | COMPLETE |
| C | Chunking & Retrieval | 2 | |
| D | Text RAG E2E | 3 | IMPLEMENTED |
| E | Image Pipeline | 4-6 | |
| F | QA & Hardening | 7-8 | |

### EPIC B: Data Readiness

EPIC B artifacts are generated by `src/utils/generate_epic_b_artifacts.py` and include:
- Benchmark query sets and ground truth (56 queries)
- Screenshot metadata and OCR annotations (21 screenshots)

**Regenerate artifacts**: `python -m src.utils.generate_epic_b_artifacts`

**Full documentation**: [docs/DATA_ARTIFACTS.md](docs/DATA_ARTIFACTS.md)

### EPIC D: Text RAG E2E

EPIC D adds a text question flow with retrieval, prompt assembly, citations, safe fallback, and
structured JSONL interaction logs.

**Demo**:
```bash
python scripts/run_text_rag_demo.py "INVALID_CREDENTIALS Caps Lock ten dang nhap"
```

**Full documentation**: [docs/EPIC_D_TEXT_RAG_E2E.md](docs/EPIC_D_TEXT_RAG_E2E.md)

## Đóng góp

Xem [CONTRIBUTING.md](CONTRIBUTING.md) để biết quy trình đóng góp.

## License

Internal project — HueCIT 2026
